...
def analyze_article(article, company):
    """Use Claude to summarize, classify sentiment, and identify substantive gaps."""
...
def stock():
    ...
    raw_articles = get_news(company.get("companyName") or ticker, limit=4)

    if raw_articles:
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = [executor.submit(analyze_article, art, company) for art in raw_articles]
